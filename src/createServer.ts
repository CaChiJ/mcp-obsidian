...
import { SemanticSearchService, VectorStore } from "./search/index.js";
import { OnnxAdapter } from "./embedding/onnx-adapter.js";
...
export function createServer(vaultPath: string, options: CreateServerOptions = {}): Server {
  ...
  const vectorStore = new VectorStore(resolvedVaultPath, embeddingBackend);
  const searchService = new SemanticSearchService(resolvedVaultPath, pathFilter, vectorStore);
